 ...
 */
	...
	add_action( 'activated_plugin', 'ghatb_bfp_1st_dev', 0 );
	...
	if ( $wp_version >= 3.2 )

		{
			...
			add_action( 'admin_head', 'ghatb_bfp_admin_back_menu_remove_dev' );

		}

	function ghatb_bfp_warning_notice_dev()
	...
	add_action( 'admin_notices', 'ghatb_bfp_warning_notice_dev' );

	function ghatb_bfp_admin_back_menu_remove_dev()
	...
	function ghatb_bfp_admin_styles_dev()
	...
	add_action( 'admin_print_styles', 'ghatb_bfp_admin_styles_dev', 21 );

	function ghatb_bfp_login_header_dev()

		{
			...
			echo '
			<!--Start Plugin Global Hide Admin Tool Bar Bruteforce Code DEV-->
			<style type="text/css">
			...
			<!--/ End Plugin Global Hide Admin Tool Bar Bruteforce Code DEV-->
			';
	...
	if ( $wp_version >= 3.3 )

		{
			...
			add_action( 'in_admin_header', 'ghatb_bfp_login_header_dev' );

			add_filter( 'show_wp_pointer_admin_bar', '__return_false' );

		}

	function ghatb_bfp_wp_toolbar_init_dev()
	...
	add_filter( 'init', 'ghatb_bfp_wp_toolbar_init_dev', 9 );

	function ghatb_bfp_remove_profile_option_dev()
	...
	add_action( 'admin_print_styles-profile.php', 'ghatb_bfp_remove_profile_option_dev' );
	...
	function ghatb_bfp_replace_warning_notice_dev()

		{
			if ( has_filter( 'admin_notices', 'ghatb_bfp_warning_notice' ) )

				{ 
					remove_filter( 'admin_notices', 'ghatb_bfp_warning_notice' ); 
				}
		}

	add_filter( 'in_admin_header', 'ghatb_bfp_replace_warning_notice_dev', 1 );

	function ghatb_bfp_replace_login_header_dev()
	...
	add_filter( 'in_admin_header', 'ghatb_bfp_replace_login_header_dev', 1 );

	function ghatb_bfp_prml_dev( $links, $file )
	...
	add_filter( 'plugin_row_meta', 'ghatb_bfp_prml_dev', 10, 2 );

	function ghatb_bfp_shfl_dev()
	...
	add_action( 'wp_head', 'ghatb_bfp_shfl_dev', 0 );
	add_action( 'wp_footer', 'ghatb_bfp_shfl_dev', 0 );
